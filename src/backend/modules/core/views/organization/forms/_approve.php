...
$this->title = Lang::t('Approve {resource}', ['resource' => $controller->resourceLabel]);
...
<div class="modal-body">
    ...
    <?= $form->field($model, 'date_approved', [])->textInput([
        'class' => 'form-control show-datepicker',
        'data-min-date' => $model->application_date,
        'data-max-date' => DateUtils::getToday(),
    ]) ?>
    <?= $form->field($model, 'approval_notes', []) ?>
    <?= $form->field($model, 'membership_end_date', [])->textInput([
        'class' => 'form-control show-datepicker',
        'data-min-date' => DateUtils::getToday(),
    ]) ?>
</div>

<div class="modal-footer">
    <button type="submit" class="btn btn-primary">
        <?= Lang::t($model->isNewRecord ? 'Create' : 'Approve') ?>
    </button>
