...
<div class="modal-body">
    ...
    <?php if ($model->isNewRecord): ?>
        <?= $this->render('_fileField', ['model' => $model]) ?>
    <?php endif; ?>
    <?= $form->field($model, 'document_no', []) ?>
    <?= $form->field($model, 'doc_type_id')->widget(Select2::class, [
        'data' => RegistrationDocumentType::getOrgListData($model->org->business_type,$model->org->business_entity_type),
        'modal' => true,
    ...
    <?= $form->field($model, 'start_date', [])->textInput(['class' => 'form-control show-datepicker']) ?>
    <?= $form->field($model, 'renewal_date', [])->textInput(['class' => 'form-control show-datepicker']) ?>
    <?php if (!$model->isNewRecord): ?>
