        ...
        <a class="nav-link<?= empty($animalType) ? ' active' : '' ?>"
           href="<?= Url::to(['index', 'animal_type' => null, 'country_id' => [!empty($country) ? $country->id : null]]) ?>">
            ...
            <span class="badge badge-secondary badge-pill">
                <?=Yii::$app ->formatter->asDecimal(CountriesDashboardStats::getAnimalCounts((!empty($country) ? $country->id : null))) ?>
            </span>
        </a>
    </li>
        ...
        <li class="nav-item">
            <a class="nav-link<?= $animalType == $value ? ' active' : '' ?>"
               href="<?= Url::to(['index', 'animal_type' => $value, 'country_id' => !empty($country) ? $country->id : null]) ?>">
                ...
                <span class="badge badge-secondary badge-pill">
                    <?=Yii::$app ->formatter->asDecimal(CountriesDashboardStats::getAnimalCounts((!empty($country) ? $country->id : null), $value)) ?>
                </span>
            </a>
        </li>
    <?php endforeach; ?>
